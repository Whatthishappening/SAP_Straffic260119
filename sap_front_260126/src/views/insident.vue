...
const onBulkStationSelected = async (stationData) => {
  ...
  if (confirm(`${selectedIds.value.length}개의 이슈 위치를 [${stationData.line_name} ${stationData.station_name}]으로 변경하시겠습니까?`)) {
    try {
      const payload = {
        ...
        station_id: Number(stationData.station_id) // 문자열 "" 방지를 위해 강제 숫자 변환
      };
